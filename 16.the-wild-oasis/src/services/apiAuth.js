...
export async function updateUser({password, fullName, avatar}){
  ...
  let avatarName = `avatar-${data.user.id}-${Math.random().toString().replaceAll(".", "0")}`;

  const {error: storageError} = await supabase.storage.from("avatars").upload(avatarName, avatar);
  if (storageError) throw new Error(storageError.message)
  const {data: updatedUser, error:error2} = await supabase.auth.updateUser({
    data: {
      avatar: `https://zbzfewntbmhnaeqapoxr.supabase.co/storage/v1/object/public/avatars/${avatarName}`
    }
  })
